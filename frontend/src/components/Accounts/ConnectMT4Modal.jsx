            ...
            <div className="bg-dark-900 rounded-lg p-3 border border-brand-500/30">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleCopy} className="btn-secondary px-2 py-1.5 flex-shrink-0">
                  {copied ? <ClipboardDocumentCheckIcon className="w-4 h-4 text-brand-400" /> : <ClipboardIcon className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <div className="text-xs text-slate-500 space-y-1">
              <p>1. Buka MT4 dan pasang EA SeizeBridge ke chart</p>
              <p>2. Isi ServerUrl dengan URL Railway kamu</p>
              <p>3. Paste token di atas ke input BridgeToken</p>
      ...
      <div className="w-full max-w-md bg-dark-800 border border-slate-700 rounded-2xl shadow-2xl animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            ...
            <input type="number" className="input-field" placeholder="e.g. 87200828" value={form.login} onChange={(e) => setForm({ ...form, login: e.target.value })} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              Server <span className="text-danger-400">*</span>
            </label>
            <input type="text" className="input-field" placeholder="e.g. Exness-Real30, ICMarkets-Live02" value={form.server} onChange={(e) => setForm({ ...form, server: e.target.value })} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
